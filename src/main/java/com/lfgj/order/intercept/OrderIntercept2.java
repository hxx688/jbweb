...
public class OrderIntercept2 extends PageIntercept{
	...
	public void queryBefore(AopContext ac) {
		...
		if(!Func.isEmpty(startDate)){
			sql += " and DATE_FORMAT(t.sale_time,'%Y-%m-%d') >= '" + startDate + "'";
		}
		
		if(!Func.isEmpty(endDate)){
			sql += " and DATE_FORMAT(t.sale_time,'%Y-%m-%d') <= '" + endDate + "'";
		}
		...
		if(member_id>0){
			Member member = Blade.create(Member.class).findById(member_id);
			if(member.getIs_agent() == 1){
				sql += " and find_in_set("+member_id+", u.parent_ids)";
			}else{
				sql += " and t.person_id = "+member_id;
			}
		}
	...
	public void queryAfter(AopContext ac) {
		...
		String sql = "SELECT COALESCE (SUM(t.earn), 0) amount,COALESCE (SUM(t.fee), 0) fee FROM dt_orders t,lf_member u WHERE t.person_id = u.id and u.is_agent=0";
		sql += ac.getCondition();
		sql += ac.getSqlEx();
