 ...
 */
package com.rrgy.core.beetl.tag;
	...
	public void render() {
		try {
			...
			if (type.equals("dict")) {
				...
				sql = "select ID,name as TEXT from  TFW_ROLE where id>0";
				...
				CACHE_NAME = ConstCache.SEL_CACHE;
				...
				sid = "_" + sid;
				sb.append("<select class=\"form-control\" " + required + " name=\"" + token + name + "\" id=\"" + sid + "\">");
			...
			List<Map> dict = new ArrayList<Map>();
			if (type.equals("sql")){
				dict = Db.selectList(_sql, _modelOrMap, new AopContext(), _intercept);
			}else{
				dict = CacheKit.get(CACHE_NAME, ConstCacheKey.DICT + type + "_" + code + "_" + ShiroKit.getUser().getId(), new ILoader() {
					@Override
					public Object load() {
