...
public class DbManager {
	...
	*/
	...
	public <T> BladePage<T> paginate(String sqlTemplate, String sqlCount, Class<T> clazz, Object paras, int pageNum, int pageSize){
		...
		if(ix!=-1){
			String lastSql = sqlTemplate.substring(ix+1);
			String sql = "SELECT COUNT(*) CNT "+lastSql;
			count = queryInt((StrKit.isBlank(sqlCount) ? sql : sqlCount), paras).longValue();	
		}else{
			count = queryInt((StrKit.isBlank(sqlCount) ? "SELECT COUNT(*) CNT FROM (" + sqlTemplate + ") a" : sqlCount), paras).longValue();	
		}
